...
<h2>Examining YouTube Ad Types and Durations</h2>

<p>As seen in the bar graph below the most common form of ad seen on YouTube from my small sample size was the skippable video ad with 16 out of the 20 ad types being skippable video ads. The other two were short unskippable ads and display ads that normally in the form of a banner and were not over top of the video.</p>
...
<p>The total ad length for all ads recorded in the 20 samples was `r paste(total_length)` seconds making the average for all `r paste(total_length / 20)` seconds however it would be interesting to see if it differs in each category as some types of ads may tend to be longer than others. In the next bar graph it measures the mean ad length by each category. The highest is education/courses ads however this was only a single ad that is adding to this data `r paste("which was ", max_value, "seconds")`. The next highest is Tech ads which has a decent sample size of 4 and still has a relatively high ad length.</p>

<h2>Mean Ad Length by Category</h2>
